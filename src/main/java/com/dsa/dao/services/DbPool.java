...
public final class DbPool {
  ...
  public static void initializeDbPool(String dbClassName, String url, String user, String password, int maxConnections){
    ...
    if(connectionPool==null){
      try{
        Class.forName(dbClassName); // Class.forName(dbClassName).newInstance();
      }catch(ClassNotFoundException e) { // }catch(ClassNotFoundException | IllegalAccessException | InstantiationException e) {
        e.printStackTrace();
